...
<c:if test="<%= fullView %>">
	<div id="<portlet:namespace />fullViewMarkAllAsRead"></div>
</c:if>
...
<c:if test="<%= !fullView %>">
	...
	<li class="bottom message">
		...
		<c:if test="<%= !userNotificationEventIds.isEmpty() %>">
			<div id="<portlet:namespace />dropDownMarkAllAsRead"></div>
		</c:if>
	</li>
</c:if>
...
<aui:script use="aui-base,aui-io-plugin-deprecated">
	...
	<portlet:renderURL var="refreshURL" windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>">
	...
	var createMarkAllAsReadNode = function() {
		<c:if test="<%= !userNotificationEventIds.isEmpty() && (start == 0) %>">
			...
			var markAsAllReadNode = A.Node.create('<a class="dismiss-notifications" href="<%= dismissNotificationsURL %>"><%= LanguageUtil.format(pageContext, "mark-all-as-read-x", String.valueOf(userNotificationEventIds.size()), false) %></a>');
			...
			if (userNotificationsList) {
				replaceNode(userNotificationsList, '#<portlet:namespace/>fullViewMarkAllAsRead');
			}

			var dockbarUserNotificationsList = A.one('.dockbar-user-notifications .user-notifications-list');

			if (dockbarUserNotificationsList) {
				replaceNode(dockbarUserNotificationsList, '#<portlet:namespace/>dropDownMarkAllAsRead');
			}
		</c:if>
	};
	...
	if (userNotificationsList) {
		...
		userNotificationsList.delegate(
			'click',
			function(event) {
				Liferay.Notifications.dismissNotifications(event, false, '<%= refreshURL %>');
			},
		...
		userNotificationsList.delegate(
			'click',
			function(event) {
				Liferay.Notifications.dismissNotifications(event, true, '<%= refreshURL %>');
			},
